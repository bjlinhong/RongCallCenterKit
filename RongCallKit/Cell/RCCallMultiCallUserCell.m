...
- (void)setModel:(RCCallUserCallInfoModel *)model status:(RCSCallStatus)callStatus {
    _model = model;
    ...
    if ((callStatus == RCSCallStatusInviting || callStatus == RCSCallStatusConnected) && model.profile.mediaType == RCSCallMediaTypeVideo) {
    } else {
        self.headerImageView.layer.borderWidth = 1;
        self.headerImageView.layer.borderColor = [UIColor whiteColor].CGColor;
    ...
    if (callStatus == RCSCallStatusIncoming || model.profile.mediaType == RCSCallMediaTypeVideo) {
        nameLabelHeight = 0;
        insideMargin = 0;
    }

    if (model.profile.mediaType == RCSCallMediaTypeAudio) {
    ...
    if (model.profile.callStatus == RCSCallStatusInviting ||
        model.profile.callStatus == RCSCallStatusIncoming) {
...
- (void)resetLayout:(CGFloat)nameLabelHeight insideMargin:(CGFloat)insideMargin {
    ...
    if (self.callStatus == RCSCallStatusIncoming) {
        self.statusView.hidden = YES;
    } else {
        self.statusView.hidden = NO;

        if (_model.profile.callStatus == RCSCallStatusIncoming ||
            _model.profile.callStatus == RCSCallStatusInviting) {
            self.statusView.hidden = NO;
        } else {
            self.statusView.hidden = YES;
        }
