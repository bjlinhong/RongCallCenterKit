...
- (void)setDataModel:(RCMessageModel *)model {
    ...
    if ([model.content isMemberOfClass:[RCSCallSummaryMessage class]]) {
        RCSCallSummaryMessage *message = (RCSCallSummaryMessage *)model.content;

        NSString *status = nil;
        if (message.connectedTime > 0) {
            status = RCCallKitLocalizedString(@"VoIPCallHasEnd");
        } else if (message.hangupReason == RCSCallDisconnectReasonReject ||
                   message.hangupReason == RCSCallDisconnectReasonRemoteReject) {
            status = RCCallKitLocalizedString(@"VoIPCallHasReject");
        } else if (message.hangupReason == RCSCallDisconnectReasonCancel) {
            status = RCCallKitLocalizedString(@"VoIPCallHasCancel");
        } else if (message.hangupReason == RCSCallDisconnectReasonRemoteCancel) {
            status = RCCallKitLocalizedString(@"VoIPCallNoResponse");
        } else if (message.hangupReason == RCSCallDisconnectReasonAcceptByOtherClient) {
        ...
        if (message.mediaType == RCSCallMediaTypeAudio) {
