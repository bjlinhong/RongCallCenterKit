...
- (void)didTapCameraCloseButton {
    [self resetLayout:self.callSession.isMultiCall mediaType:RCSCallMediaTypeAudio callStatus:self.callSession.callStatus];
}
...
- (void)resetLayout:(BOOL)isMultiCall mediaType:(RCSCallMediaType)mediaType callStatus:(RCSCallStatus)sessionCallStatus {
    ...
    if (mediaType == RCSCallMediaTypeAudio) {
        ...
        } else if (callStatus == RCSCallStatusInviting || callStatus == RCSCallStatusIncoming) {
            self.remotePortraitView.alpha = 0.5;
            self.statusView.hidden = NO;
            self.blurView.hidden = NO;
        } else {
...
- (void)backgroundSingleViewClicked {
    if (self.callSession.mediaType == RCSCallMediaTypeVideo && self.callSession.callStatus == RCSCallStatusConnected) {
