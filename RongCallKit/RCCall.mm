...
#if PUBLIC
#else
            [[RCSCallManager sharedManager] setApplePushKitEnable:YES];
#endif
...
- (BOOL)isAudioCallEnabled:(RCConversationType)conversationType {
    return [[RCSCallManager sharedManager] isAudioCallEnabled:conversationType];
}

- (BOOL)isVideoCallEnabled:(RCConversationType)conversationType {
    return [[RCSCallManager sharedManager] isVideoCallEnabled:conversationType];
}

- (void)startSingleCall:(NSString *)targetId mediaType:(RCSCallMediaType)mediaType {
...
- (void)checkSystemPermission:(RCSCallMediaType)mediaType success:(void (^)(void))successBlock {
...
- (BOOL)preCheckForStartCall:(RCSCallMediaType)mediaType {
    RCSCallSession *currentCallSession = [RCCall sharedRCCall].currentCallSession;
    if (currentCallSession && currentCallSession.mediaType == RCSCallMediaTypeAudio) {
        [self loadErrorAlertWithoutConfirm:RCCallKitLocalizedString(@"VoIPAudioCallExistedWarning")];
        return NO;
    } else if (currentCallSession && currentCallSession.mediaType == RCSCallMediaTypeVideo) {
...
- (void)didReceiveCall:(RCSCallSession *)callSession {
    ...
    if (!callSession.isMultiCall) {
        ...
        if (callSession.mediaType == RCSCallMediaTypeAudio) {
    ...
    if ([RCIMClient sharedRCIMClient].pushProfile.isShowPushContent ||
        ...
        if (pushConfig && pushConfig.pushContent && pushConfig.pushContent.length != 0 &&
            ...
            if (isCancelCall) {
                ...
                if (userInfo) {
                    pushContent =
                    [NSString stringWithFormat:self.currentCallSession.mediaType == RCSCallMediaTypeAudio ?
                                                       RCCallKitLocalizedString(@"VoIPCall_invite_audio_push_Content") :
                                                       RCCallKitLocalizedString(@"VoIPCall_invite_video_push_Content"),
                                                   userInfo.name];
                } else {
                    pushContent = self.currentCallSession.mediaType == RCSCallMediaTypeVideo ?
