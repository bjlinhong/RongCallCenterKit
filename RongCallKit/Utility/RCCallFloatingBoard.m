...
@interface RCCallFloatingBoard () <RCSCallSessionDelegate>

@property (nonatomic, strong) NSTimer *activeTimer;
@property (nonatomic, copy) void (^touchedBlock)(RCSCallSession *callSession);
...
+ (void)startCallFloatingBoard:(RCSCallSession *)callSession
              withTouchedBlock:(void (^)(RCSCallSession *callSession))touchedBlock {
...
- (void)updateActiveTimer {
    ...
    if (self.callSession.callStatus == RCSCallStatusConnected && ![self isVideoViewEnabledSession]) {
...
- (BOOL)isVideoViewEnabledSession {
    if (self.callSession.mediaType != RCSCallMediaTypeAudio && !self.callSession.multiCall) {
        return YES;
    } else {
        return NO;
    }
